...
def get_text():
    ...
    casing = request.args.get('casing', 'default') # Get the new casing parameter

    word_pool = WORDS.copy()
    if nums:
        word_pool.extend(NUMBERS)

    num_words = 200 if mode == 'time' else count
    ...
    processed_list = []
    if casing == 'lowercase':
        processed_list = [word.lower() for word in text_list]
    elif casing == 'uppercase':
        processed_list = [word.capitalize() for word in text_list]
    else: 
        processed_list = text_list

    return jsonify({'text': ' '.join(processed_list)})
...
@login_required
def get_stats_data():
    results = TestResult.query.filter_by(user_id=current_user.id).order_by(TestResult.timestamp.asc()).limit(50).all()
    labels = [res.timestamp.strftime('%b %d') for res in results]
    wpm_data = [res.wpm for res in results]
    accuracy_data = [res.accuracy for res in results]
